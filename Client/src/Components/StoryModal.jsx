...
const StoryModal = ({ setShowModal, fetchStories }) => {
  ...
  const MAX_VEDIO_DURATION = 60;
  const MAX_VEDIO_SIZE_MB = 50;

  const handleMediaUpload = (e) => {
    const file = e.target.files?.[0];
    if (file) {
      if (file.type.startsWith("video")) {
        if (file.size > MAX_VEDIO_SIZE_MB * 1024 * 1024) {
          toast.error(`Video file size can't exceed ${MAX_VEDIO_SIZE_MB}MB.`);
          setMedia(null);
          setPreviewUrl(null);
          return;
        }
        ...
        video.unloadedmetadata = () => {
          window.URL.revokeObjectURL(video.src)

          if (video.duration > MAX_VEDIO_DURATION) {
        ...
        <div className="flex mt-4 gap-2">
          {
            bgColors.map((color) => (
              <button
                key={color} className="w-6 h-6 rounded-full ring cursor-pointer"
