...
const StoryViewer = ({ stories, currentIndex, setCurrentIndex }) => {
  ...
  const renderContent = () => {
    switch (viewStory.media_type) {
      ...
      case 'video':
        return (
          <video onEnded={() => setCurrentIndex(null)} src={viewStory.media_url} controls autoPlay className="w-full h-full object-contain" />
        );
      case 'text':
        return (
  ...
  return (
    <div className="fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center" style={{ backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color : '#000000' }}>
      ...
      {viewStory.media_type !== 'video' && (
        <div className="absolute top-0 w-full h-1 bg-gray-700 z-50">
      ...
      {/* User Info -Top Left */}
      <div className="absolute top-4 left-4 z-50 flex items-center space-x-3 p-2 px-4 sm:p-4 sm:px-8 backdrop-blur-2xl rounded bg-black/50">
      ...
      {/* Close Button */}
      <button onClick={handleClose} className="absolute top-4 right-4 z-50 text-white text-3xl font-bold focus:outline-none">
